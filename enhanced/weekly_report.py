...
def generate_weekly_report():
    ...
    state_lines = "\n".join(
        f"- {state}: {percentage:.1f}%" for state, percentage in avg_state_times.items() if percentage > 0
    )
    high_lines = (
        "\n".join(f"- {i['description']} ({i['timestamp']})" for i in high_insights)
        if high_insights else "None"
    )
    medium_lines = (
        "\n".join(f"- {i['description']} ({i['timestamp']})" for i in medium_insights)
        if medium_insights else "None"
    )
